...
public class NebulaSky : CustomSky
{
	...
	public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
	{
		if (maxDepth >= float.MaxValue && minDepth < float.MaxValue)
		{
			...
			spriteBatch.Draw(_planetTexture.Value, vector + new Vector2(-200f, -200f) + flameParticle, null, Color.White * 0.9f * _fadeOpacity, 0f, new Vector2(_planetTexture.Width() >> 1, _planetTexture.Height() >> 1), 1f, SpriteEffects.None, 1f);
		}
		int num = -1;
		int i = 0;
		...
		for (int j = num; j < i; j++)
		{
			...
			if (player.Contains((int)position.X, (int)position.Y))
			{
				...
				spriteBatch.Draw(_beamTexture.Value, position, null, Color.White * 0.2f * scaleFactor * _fadeOpacity, 0f, Vector2.Zero, new Vector2(num3 / 70f, num3 / 45f), SpriteEffects.None, 0f);
				int num4 = 0;
				for (float num5 = 0f; num5 <= 1f; num5 += 0.03f)
				{
					float num6 = 1f - (num5 + Main.GlobalTimeWrappedHourly * 0.02f + (float)Math.Sin(j)) % 1f;
					spriteBatch.Draw(_rockTextures[num4].Value, position + new Vector2((float)Math.Sin(num5 * 1582f) * (num3 * 0.5f) + num3 * 0.5f, num6 * 2000f), null, Color.White * num6 * scaleFactor * _fadeOpacity, num6 * 20f, new Vector2(_rockTextures[num4].Width() >> 1, _rockTextures[num4].Height() >> 1), 0.9f, SpriteEffects.None, 0f);
