...
public class CoinLossRevengeSystem
{
	public class RevengeMarker
	{
		...
		public bool DrawMapIcon(SpriteBatch spriteBatch, Vector2 mapTopLeft, Vector2 mapX2Y2AndOff, Rectangle? mapRect, float mapScale, float drawScale, int gameTime)
		{
			...
			Texture2D packet = TextureAssets.MapDeath.Value;
			packet = ((_coinsValue < 100) ? TextureAssets.Coin[0].Value : ((_coinsValue < 10000) ? TextureAssets.Coin[1].Value : ((_coinsValue >= 1000000) ? TextureAssets.Coin[3].Value : TextureAssets.Coin[2].Value)));
