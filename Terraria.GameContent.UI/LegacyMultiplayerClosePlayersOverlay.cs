		...
		for (int i = 0; i < 255; i++)
		{
			...
			if (num5 > 0f)
			{
				...
				if (!(num13 > (float)num3))
				{
					...
					ChatManager.DrawColorCodedStringWithShadow(tile, FontAssets.MouseText.Value, name, namePlatePos + new Vector2(0f, -40f), namePlateColor, 0f, Vector2.Zero, Vector2.One);
				}
			}
			else
			{
