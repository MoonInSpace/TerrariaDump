...
public class CustomCurrencySingleCoin : CustomCurrencySystem
{
	...
	public override void DrawSavingsMoney(SpriteBatch sb, string text, float shopx, float shopy, long totalCoins, bool horizontal = false)
	{
		...
		Texture2D num2 = TextureAssets.Item[num].Value;
		if (horizontal)
		{
			_ = 99;
			Vector2 value = new Vector2(shopx + ChatManager.GetStringSize(FontAssets.MouseText.Value, text, Vector2.One).X + 45f, shopy + 50f);
			sb.Draw(num2, value, null, Color.White, 0f, num2.Size() / 2f, CurrencyDrawScale, SpriteEffects.None, 0f);
			Utils.DrawBorderStringFourWay(sb, FontAssets.ItemStack.Value, totalCoins.ToString(), value.X - 11f, value.Y, Color.White, Color.Black, new Vector2(0.3f), 0.75f);
		}
		else
		{
			...
			Utils.DrawBorderStringFourWay(sb, FontAssets.ItemStack.Value, totalCoins.ToString(), shopx + (float)num3, shopy + 75f, Color.White, Color.Black, new Vector2(0.3f), 0.75f);
		}
	}
