...
public class GroupOptionButton<T> : UIElement, IGroupOptionButton
{
	...
	protected override void DrawSelf(SpriteBatch spriteBatch)
	{
		...
		Utils.DrawSplicedPanel(spriteBatch, _BasePanelTexture.Value, (int)color.X, (int)color.Y, (int)color.Width, (int)color.Height, 10, 10, 10, 10, Color.Lerp(Color.Black, value, FadeFromBlack) * item);
		if (isSelected && ShowHighlightWhenSelected)
		{
			Utils.DrawSplicedPanel(spriteBatch, _selectedBorderTexture.Value, (int)color.X + 7, (int)color.Y + 7, (int)color.Width - 14, (int)color.Height - 14, 10, 10, 10, 10, Color.Lerp(value, Color.White, _whiteLerp) * item);
		}
		if (_hovered)
		{
			Utils.DrawSplicedPanel(spriteBatch, _hoveredBorderTexture.Value, (int)color.X, (int)color.Y, (int)color.Width, (int)color.Height, 10, 10, 10, 10, _borderColor);
		}
		if (_iconTexture != null)
		{
			...
			spriteBatch.Draw(_iconTexture.Value, new Vector2(color.X + 1f, color.Y + 1f), color2);
		}
	}
