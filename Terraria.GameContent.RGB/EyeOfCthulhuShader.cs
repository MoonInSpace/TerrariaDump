...
public class EyeOfCthulhuShader : ChromaShader
{
	...
	private void ProcessLowDetail(RgbDevice device, Fragment fragment, EffectDetailLevel quality, float time)
	{
		for (int num = 0; num < fragment.Count; num++)
		{
			...
			Vector4 num2 = Vector4.Lerp(_veinColor, _eyeColor, (float)Math.Sin(time + dimensions.X * 4f) * 0.5f + 0.5f);
	...
	private void ProcessHighDetail(RgbDevice device, Fragment fragment, EffectDetailLevel quality, float time)
	{
		...
		Vector2 j = new Vector2(text * 7f - 3.5f, 0f) + fragment.CanvasCenter;
