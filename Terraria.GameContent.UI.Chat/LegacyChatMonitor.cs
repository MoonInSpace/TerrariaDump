	...
	public void DrawChat(bool drawingPlayerChat)
	{
		...
		for (int j = text; j < list; j++)
		{
			if (drawingPlayerChat || (chatLine[j].showTime > 0 && chatLine[j].parsedText.Length != 0))
			{
				...
				ChatManager.DrawColorCodedStringWithShadow(Main.spriteBatch, FontAssets.MouseText.Value, chatLine[j].parsedText, new Vector2(88f, Main.screenHeight - 30 - 28 - substitutions * 21), 0f, Vector2.Zero, Vector2.One, out hoveredSnippet);
